...
void CImageCaptureDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_KEEPLINKS, m_KeepLinks);
...
void CImageCaptureDlg::OnCapture() 
{
   ...
   ///////////////////////////////////////////////////////////////////////////////
   // The following features are new with SnagIt version 8.1

   // Hotspots (keep links)
   if ( m_KeepLinks.GetCheck() )
   {
      m_pImageCapture->HotspotType = SnagIt::shtLinksOnly;
   }
   else
   {
      m_pImageCapture->HotspotType = SnagIt::shtNone;
   }

   // Add a caption
   m_pImageCapture->Filters->Annotation->EnableCaption = VARIANT_TRUE;
   CComBSTR bstrCaption = _T("Example Caption outside image");
   m_pImageCapture->Filters->Annotation->CaptionText.Assign( bstrCaption );
   m_pImageCapture->Filters->Annotation->CaptionOptions->Placement = SnagIt::spOutsideBottom;
...
void __stdcall CImageCaptureDlg::OnSnagItState( SnagIt::snagCaptureState nStateCode )
{
   switch ( nStateCode )
   {
      case SnagIt::scsCaptureSucceeded:
      {
         ...
         // Get Coordinates; new in v8.1
         int nStartX = m_pImageCapture->CaptureResults->SelectedArea->StartX;
         int nStartY = m_pImageCapture->CaptureResults->SelectedArea->StartY;
         int nEndX = nStartX + m_pImageCapture->CaptureResults->SelectedArea->Width;
         int nEndY = nStartY + m_pImageCapture->CaptureResults->SelectedArea->Height;
         
         CString strCoords;
         strCoords.Format( "(%d, %d) - (%d, %d)", nStartX, nStartY, nEndX, nEndY );

         GetDlgItem( IDC_COORDINATES )->SetWindowText( strCoords );
         break;
      }
      case SnagIt::scsCaptureFailed:
      {
         GetDlgItem( IDC_STATUS )->SetWindowText( "Failed" );
         GetDlgItem( IDC_COORDINATES )->SetWindowText( "" );
